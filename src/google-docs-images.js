...
(function( $ ) {
  ...
  $('img').each(function (index, element) {
    ...
    $(element).next('.imgpath').html(imgpath +'<br>' + imgName + '_' + (index + 1));
    console.log('Путь к оригинальной картинке: %c' + $(element).attr('src') + '%c, переименованный правильный вариант: %c' + imgName + '_' + (index + 1), 'color: red;', 'color: inherit;', 'color: green;');
  });
